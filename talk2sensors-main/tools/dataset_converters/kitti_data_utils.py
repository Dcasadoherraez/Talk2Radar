...
def get_prompt_path(idx,
                  prefix,
                  training=True,
                  relative_path=True,
                  exist_check=True,
                  use_prefix_id=False):
    return get_kitti_info_path(idx, prefix, 'prompt', '.txt', training,
                               relative_path, exist_check, use_prefix_id)
...
def get_kitti_image_info(path,
    ...
    def map_func(idx):
        ...
        pc_info = {'num_features': 7} #zrx for vod
        calib_info = {}
        ...
        if prompt:
            ...
            prompt_info["text_prompt"] = prompt_content
            info['prompt'] = prompt_info
        ...
        if calib:
            ...
            # ]).reshape([3, 4])
            Tr_imu_to_velo = np.eye(4) # zrx for vod
